...
def sort_noise(timecourses=None, lag1=None, return_logpdf=False, method='KDE', verbose=False):
    '''
...
def local_max(xvalues, array1d, sig=None):
    ...
    if sig is not None:
        i = np.r_[True, array1d[1:] > array1d[:-1]] & np.r_[array1d[:-1] > array1d[1:], True]
        return list(compress(xvalues, i)), list(compress(array1d, i)), list(compress(sig, i))
    else:
        i = np.r_[True, array1d[1:] > array1d[:-1]] & np.r_[array1d[:-1] > array1d[1:], True]
...
def tdelay_correlation(vectors, n, max_offset=150, return_window=False):
    ...
    for i,v in enumerate(vectors):
        ...
        corr = corr[tc.size - max_offset -1:tc.size + max_offset] # crop to window
