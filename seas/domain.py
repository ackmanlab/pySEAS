...
# def getDomainMap(components, cutoff=None, blur=21,
#     minratio=1/10, savepath=None, maponly=True, filtermean=True,
#     calc_artifact_mask=True, max_loops=2, ignore_small=True):
...
#         if calc_artifact_mask:
#             if artifact_components.sum() == 0:
#                 output['artifact_mask'] = np.zeros(shape)

#             else:
#                 print('calculating artifact mask..')
#                 artifact_vector = eig_vec[:,
#                         np.where(artifact_components == 1)[0]].max(axis=1)

#                 thresh = np.sort(artifact_vector)[int(artifact_vector.size * 0.95)]
#                 artifact_mask_vector = np.zeros(artifact_vector.shape)
#                 artifact_mask_vector[np.where(artifact_vector > thresh)] = 1

#                 if roimask is not None:
#                     artifact_map = np.empty(shape)
#                     artifact_map[:] = np.NAN
#                     artifact_map.flat[maskind] = artifact_vector

#                     artifact_mask = np.zeros(shape)
#                     artifact_mask.flat[maskind] = artifact_mask_vector

#                 else:
#                     artifact_map = np.reshape(artifact_vector, shape)
#                     artifact_mask = np.reshape(artifact_mask_vector, shape)

#                 output['artifact_mask'] = artifact_mask
#                 # output['artifact_map'] = artifact_map

#                 if not maponly:
#                     artifact_timecourse = wbpca.rebuildMeanTimecourse(components,
#                         mask=artifact_mask, invert_artifact=True, include_noise=False,
#                         include_zero=False)[0]
#                     output['artifact_timecourse'] = artifact_timecourse
...
#                 (artifact_components + noise_components) == 0)[0]
#         else:
...
#                 roislice = np.delete(
#                     eig_vec[np.where(
#                             cc.flat[maskind] == n+1)[0],:], i, axis=1)
#             else:
#                 roislice = np.delete(
#                     eig_vec[np.where(
#                             cc.flat == n+1)[0],:], i, axis=1)
...
#         if blur is not None:
#             savepath = savepath.replace('.png', '_' + str(blur) + 'b.png')
#         # wb.saveFile(savepath, domain_ROIs.copy(), apply_cmap=False, rescale=True)

#         edges = getDomainEdges(domain_ROIs)
#         wb.saveFile(savepath, domain_ROIs.copy() + edges,
#             apply_cmap=False, rescale=True)
#         wb.saveFile(savepath.replace('.png', '_edges.png'),
#             edges, apply_cmap=False, rescale=True)

#     output['domain_ROIs'] = domain_ROIs

#     if not maponly:
#         timecourseresults = getDomainRebuiltTimecourses(domain_ROIs,
#             components, filtermean=filtermean)
...
# def getDomainRebuiltTimecourses(domain_ROIs, components, filtermean=True):

#     output = {}
#     print('\nExtracting Domain ROI Timecourses\n-----------------------')
#     ROI_timecourses = wbpca.rebuildMeanTimecourse(components, mask=domain_ROIs)
#     output['ROI_timecourses'] = ROI_timecourses

#     if filtermean:
#         mean_filtered = wbpca.filterMean(components['mean'])
...
# def getDomainEdges(domain_ROIs, clearbg=False, linepad=None):
#     edges = cv2.Canny((domain_ROIs + 1).astype('uint8'), 1, 1)

#     edges += cv2.Canny((domain_ROIs + 2).astype('uint8'), 1, 1)

#     if linepad is not None:
#         assert type(linepad) is int, 'invalid line pad.  Provide an odd integer.'
...
# def getPaddedBorders(domain_ROIs, blur, rois, bounding_box=None):

#     shape = domain_ROIs.shape
#     padmask = np.zeros((shape[0], shape[1]), dtype='uint8')

#     for i, roi in enumerate(rois):
#         mask = wb.makeMask(rois[roi], shape, bounding_box)
...
# def domainMap(domain_ROIs, values=None, returnimg=True):
...
#         return domainmap
#     else:
#         plt.imshow(domainmap); plt.colorbar(); plt.axis('off'); plt.show()
#         return

# def borderLevels(domain_ROIs, region_assignment, pad=15, flip=False, returnlayers=False):

#     macro_map = domainMap(domain_ROIs, values=region_assignment)
#     domain_borders = getDomainEdges(domain_ROIs, linepad=pad//2+1, clearbg=True).astype('float64')

#     colored_borders = domain_borders.copy() / 255
#     macro_borders = np.zeros(macro_map.shape)
#     macro_borders[:] = np.nan

#     macro_borders = getDomainEdges(domainMap(domain_ROIs, values=region_assignment), linepad=pad, clearbg=True)
...
#     meso_borders = getDomainEdges(domainMap(domain_ROIs, values=region_assignment_adjusted), linepad=(pad//4)*3, clearbg=True)
...
#         plt.imshow(getDomainEdges(domain_ROIs, clearbg=True, linepad=10), cmap='coolwarm_r', alpha=0.75)
#         plt.imshow(getDomainEdges(domain_ROIs_2, clearbg=True, linepad=10), alpha=0.75)
...
#             output = wb.scaleVideo(vidROIs, factor, 1, verbose=False)
#             output = output[0]
...
# def rollingMosaicMovie(domain_ROIs, ROI_timecourses, savepath,
#     resize_factor=1, codec=None, speed=1, fps=10, cmap=wb.default_colormap,
#     t_start=None, t_stop=None, flip=False):
...
#         frame = wb.rescaleMovie(frame, min_max=(fmin, fmax),
...
#             print('min:', fmin, 'max:', fmax, 'slope:', fslope)
...
# def mosaicMovie(domain_ROIs, ROI_timecourses, savepath=None,
#         t_start=None, t_stop=None, flip=False):
...
#         wb.saveFile(savepath, movie, rescale=True, save_cbar=True, overlay=overlay)
