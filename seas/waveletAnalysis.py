...
def waveletCoherence(tc1, tc2, fps=10, siglvl = 0.95, sigtest = 0):
    ...
    w1w2 = w1.wave * np.conj(w2.wave) #/ (np.ones([1, tc1.size]) * w1.scale[:, None])
    ...
    xwt_signif = wave_cohere_signif(([1.0]), ([1.0]), dt=1/fps, sigtest=sigtest, scale=w1.scale, x_lag1=acor2, y_lag1=acor1, mother=w1.mother, siglvl=siglvl)
    xwt_signif = xwt_signif[:, np.newaxis].dot(np.ones(w1.n)[np.newaxis, :])

    return coherency, coherence, xwt, phase, xwt_signif


def wave_cohere_signif(X, Y, dt, scale, sigtest=-1, y_lag1=-1, x_lag1=-1, siglvl=-1, dof=-1, mother=-1, param=-1):
    '''
    ...
    y_fft_theor = (1 - y_lag1 ** 2) / (1 - 2 * y_lag1 * np.cos(freq * 2 * np.pi) + y_lag1 ** 2)  # [Eqn(16)]
    y_fft_theor = y_variance * y_fft_theor  # include time-series variance

    x_fft_theor = (1 - x_lag1 ** 2) / (1 - 2 * x_lag1 * np.cos(freq * 2 * np.pi) + x_lag1 ** 2)  # [Eqn(16)]
...
def lagWaveletCoherency(tc1, tc2, fps = 10, lag = 5):
    for i in np.arange(0, lag+1):
        if i == 0:
            ...
            # create ouputs
            lag_cor = np.zeros((2*lag + 1, cor.shape[0], cor.shape[1])) * np.nan
            lag_coh = np.zeros((2*lag + 1, cor.shape[0], cor.shape[1])) * np.nan
            lag_xwt = np.zeros((2*lag + 1, cor.shape[0], cor.shape[1])) * np.nan
            lag_phase = np.zeros((2*lag + 1, cor.shape[0], cor.shape[1])) * np.nan
            lag_xwt_sig = np.zeros((2*lag + 1, cor.shape[0], cor.shape[1])) * np.nan
...
class waveletAnalysis:
    ...
    def waveletTransform(self):
        ...
        self.sig95 = self.signif[:, np.newaxis].dot(np.ones(self.n)[np.newaxis, :])  # expand signif --> (J+1)x(N) array
        self.sig95 = self.power / self.sig95  # where ratio > 1, power is significant
        return

        
    def inverseWaveletTransform(self, waveFlt = None, C_d =1.151, psi0 = np.pi**(-0.25)*0.85):
        """
        ...
        tansformConstant = ((self.dj*math.sqrt(self.cadence))/(C_d*psi0) ) # Reconstruction constant. 
    ...
    def plotPower(self, ax = None):
        ...
        colors = ['navy','slateblue','c','g', 'gold' ,'orange' ,'tomato' ,'crimson']
        ...
        CS = ax.contourf(self.time, self.period, np.log2(self.power), len(self.levels), colors=colors)
        ...
        ax.fill_between(self.time, np.max(self.period), self.coi, alpha=0.5, 
        ...
        ax.ticklabel_format(axis='y', style='plain') ## causes issues with tkinter mpl canvas
        ax.invert_yaxis()
    ...
    def waveletFilter(self, lowerPeriod=None, upperPeriod=None, movement_vector=None, sigLevel=1):
        """
        ...
        if movement_vector is not None:
            ...
            if lowerPeriod != None:
                if lowerPeriod > self.period[0]:
                    ...
                    self.waveFlt[:lower_ind, :] = self.waveFlt[:lower_ind, :]*movement_vector
            if upperPeriod != None:
                if upperPeriod < self.period[-1]:
                    ...
                    self.waveFlt[upper_ind:, :] = self.waveFlt[upper_ind:, :]*movement_vector
        ...
        # Significance filter 
        notSigInd = np.where(self.sig95 < sigLevel) # Only pass data that has power of (100% - sigThreshold). Usually sigThreshold is 95%. Was 0.25.
    ...
    def sigLost(self, slope, intercept):
        ...
        if self.verbose:
            print('We have lost approximately {0:.2f} % power over the movie'.format(loss[0] * 100))
    ...
    def familySig(self, sigList = [0.9, 0.95, 0.99, 0.999], dof=-1, sigtest=0):
        ...
        if isinstance(sigList, float):
            ...
            if sigtest == 2:
                fam_signif = np.nan
            fam_signif = wave_signif([1.0], dt=self.cadence, scale=self.scale, 
                                sigtest=sigtest, lag1=self.lag1, siglvl=sigList, dof=dof, 
                                mother=self.mother, param=self.param)

        if isinstance(sigList, list):  
            if sigtest < 2: 
                fam_signif = np.zeros((len(sigList), self.scale.shape[0])) * np.nan
            if sigtest == 2:
                fam_signif = np.zeros((len(sigList), 1)) * np.nan

            for i, sig in enumerate(sigList):
               fam_signif[i] = wave_signif([1.0], dt=self.cadence, scale=self.scale, 
                                sigtest=sigtest, lag1=self.lag1, siglvl=sig, dof=dof, 
                                mother=self.mother, param=self.param)
    ...
    def sumAcrossPeriod(self, perLim =[0,100]):
        ...
        period_sum = (self.dj * self.cadence)/cdelta * np.nansum((self.nanCOImat[l_per_lim:u_per_lim]**2 / self.period[l_per_lim:u_per_lim, None]), axis = 0)
    ...
    def globalWaveletSpectrum(self):
        if self.verbose:
            print('Assessing wavelet mother {0} at paramter {1}'.format(self.mother, self.param) )
        ...
        if nan_ind.any():
            self.gws[:nan_ind[0]] = np.nanmean(self.nanCOImat[:nan_ind[0],:], axis = 1)
            self.gws[nan_ind] = 0
        else:
        ...
        if self.period_size.shape[0] != self.period.shape[0]:
            ...
            if dif < 0:
                self.period_size = np.append(self.period_size, np.zeros(np.abs(dif)))
            else:
                self.period_size = self.period_size[:self.period.shape[0]]

        # calculate the average significance
        self.gws_sig, self.gws_sigList = self.familySig(sigList=[0.95], dof=self.period_size, sigtest=1)
        
        if self.verbose:
            print('Auto-correlation value: {0:.4g}'.format(self.lag1))
        # determine fourier wavelength
        if self.mother == 'DOG':
            self.flambda = (2 * np.pi * 1/self.period) / np.sqrt(self.param + .5)
        if self.mother == 'MORLET':
            self.flambda = (4 * np.pi * 1/self.period) / (self.param + np.sqrt(2 + np.square(self.param)))
        ...
        if self.plot:
            ...
            linetype = ['-','-.','--',':']
            fam_signif, sigList = self.familySig(dof=self.period_size, sigtest=1)
            ...
            for i in range(len(sigList)):
                if i >= len(linetype) - 1:
                    ...
                    j = i
                ax1.plot(self.period, fam_signif[i], label = sigList[i], ls = linetype[j], color='k')
            if not lgws:
                ax1.set_ylim([0,10])
            else:
            ...
            # ax1.legend()
            ax1.set_xlabel(' Wavelet {0} {1} period(s)'.format(self.mother, self.param) )
    ...
    #         plt.show()
            ...
            for i in range(len(sigList)):
                if i >= len(linetype) - 1:
                    ...
                    j = i
                ax2.plot(self.flambda, fam_signif[i], label = sigList[i], ls = linetype[j],
                         color='k')
    ...
    def averageWaveletPower(self, periodLim =[0.25, 8]):
        ...
        assert len(periodLim) == 2, 'Period limit list must only include 2 values'
        
        if self.verbose:
            print('Creating scaled average of the timeseries, \
             created with wavelet mother {0} at paramter {1}'.format(self.mother, self.param))
        ...
        if self.period_sum.shape[0] != self.time.shape[0]:
            ...
            if dif < 0:
                self.period_sum = np.append(self.period_sum, np.zeros(np.abs(dif)))
            else:
    ...
    #     sig = wavelet.signif/np.sqrt(1+((self.period_size * wavelet.cadence)/(gamma * wavelet.period)))
        self.sig_period_sum, _ = self.familySig(sigList=[0.95], dof= periodLim, sigtest=2)
    ...
    def tsSignal(self, binSig=False, periodLim = [0.5, 4]):
        ...
        self.signal_sig, _ = self.familySig(sigList=[0.95], dof= periodLim, sigtest=2)

        if self.binSig:
            bintime, binsignal = self.binSignal()
            self.slope, self.intercept = linear_regression(bintime, binsignal, verbose=self.verbose)
            self.signalloss = self.sigLost(self.slope, self.intercept)
        else:
            self.slope, self.intercept = linear_regression(self.time, self.signal, verbose=self.verbose)
            self.signalloss = self.sigLost(self.slope, self.intercept)

        if self.plot:
            plt.plot(self.time, self.signal, label='signal power')
            plt.plot(self.time, np.squeeze(np.ones(self.n) * self.signal_sig), 'k--', label='{0} sign level'.format(self.siglvl))
            abline(self.slope, self.intercept, self.n * self.cadence, label='trend', color=None)
            if self.binSig:
                plt.scatter(bintime, binsignal, color ='r', label= 'binvalues')
            plt.xlabel('time(s)')
            plt.ylabel('summed power (period: {0} - {1})'.format(periodLim[0], periodLim[1]))
            plt.legend()
            plt.show()


    def noiseFilter(self, lowerPeriod=None, upperPeriod = 10, movement_vector = None, sigLevel=0.25):

        if lowerPeriod is None:
            lowerPeriod = 2*self.cadence #nyquist sampling rate

        if movement_vector is not None:
            movement_vector = np.array(np.squeeze(movement_vector), dtype = 'float64')
            assert movement_vector.shape[0] == self.n, 'Movement vector is not the same size as the data'
            
        self.waveletFilter(lowerPeriod=lowerPeriod, upperPeriod=upperPeriod, movement_vector=movement_vector, sigLevel=sigLevel)
        self.inverseWaveletTransform()
        filtData = (self.dataFlt*np.std(self.dataCopy, ddof=1)) + np.mean(self.dataCopy)
        ...
        if self.plot:
            ...
            ax2.plot(self.time, self.dataCopy - filtData, color='orange', label='residual')
            ax2.set_ylabel('dfof')
    ...
    def waveletEventDetection(self):
        ...
        self.waveletFilter(lowerPeriod = 2 * self.cadence, upperPeriod = 100, sigLevel=1)
        self.inverseWaveletTransform()
        ...
        if self.plot:
            fig = plt.figure(num=None, figsize=(20, 3*4), dpi=80, facecolor='w', edgecolor='k')
            ...
            ax1.legend()
            ax2.plot(self.time, self.data-self.dataFlt, label='residuals', c='orange')
            ax2.legend()
